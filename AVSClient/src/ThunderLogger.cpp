...
AVSRedirected::AVSRedirected(const string& text)
    : _text(WPEFramework::Core::ToString(text))
{
}

inline const char* AVSRedirected::Data() const
{
    return (_text.c_str());
}

inline uint16_t AVSRedirected::Length() const
{
...
void ThunderLogger::prettyTrace(const std::string& stringToPrint) {
    // TODO: apply a "pretty" formatting
    trace(stringToPrint);
}
...
void ThunderLogger::emit(
    ...
    TRACE(AVSRedirected,(ss.str().c_str()));
}

std::shared_ptr<Logger> getThunderLogger() {
    return ThunderLogger::instance();
}
