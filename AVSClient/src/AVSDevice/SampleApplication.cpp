...
std::unique_ptr<SampleApplication> SampleApplication::create(
    ...
    if (!clientApplication->initialize(service, consoleReader, alexaClientConfig, smartScreenConfig, pathToInputFolder, audiosource, logLevel)) {
...
SampleAppReturnCode SampleApplication::exec(InputCommand command) {
#ifdef ENABLE_THUNDER_INPUT_MANAGER
    return m_thunderInputManager->exec(command);
#else
    return SampleAppReturnCode::ERROR;
#endif
}
...
bool SampleApplication::initialize(
     ...
     * if requested.
     */
    std::shared_ptr<alexaClientSDK::avsCommon::utils::logger::Logger> thunderLogger = alexaClientSDK::sampleApp::getThunderLogger();
    ...
    if (!m_ringtoneMediaPlayer || !ringtoneSpeaker) {
        alexaClientSDK::sampleApp::ThunderLogger::trace("Failed to create media player for ringtones!");
        return false;
    }
    ...
    if (!m_capabilitiesDelegate) {
        alexaClientSDK::sampleApp::ThunderLogger::trace("Creation of CapabilitiesDelegate failed!");
        return false;
    }
    ...
    std::shared_ptr<ThunderVoiceHandler::InteractionHandler> aspInputInteractionHandler = nullptr;
...
#error "No audio input provided"
        ...
        aspInputInteractionHandler = ThunderVoiceHandler::InteractionHandler::Create();
        if(!aspInputInteractionHandler) {
            ACSDK_CRITICAL(LX("Failed to create Interaction Handler!"));
            return false;
        }

        aspInput = ThunderVoiceHandler::create(sharedDataStream, service, audiosource, aspInputInteractionHandler);
...
bool SampleApplication::jsonConfigToStream(std::vector<std::shared_ptr<std::istream>>& streams, const std::string& configFile)
{
    if (configFile.empty()) {
        alexaClientSDK::sampleApp::ThunderLogger::trace("Config filename is empty!");
        return false;
    }
    ...
    if (!configInFile->good()) {
        ...
        alexaClientSDK::sampleApp::ThunderLogger::trace("Failed to read config file " + configFile);
        return false;
    }
