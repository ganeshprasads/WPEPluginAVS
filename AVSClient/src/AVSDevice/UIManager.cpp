...
void UIManager::onSpeakerSettingsChanged(
    ...
    m_executor.submit([source, type, settings]() {
        ...
        oss << "SOURCE:" << source << " TYPE:" << type << " VOLUME:" << static_cast<int>(settings.volume)
            << " MUTE:" << settings.mute;
        ThunderLogger::prettyTrace(oss.str());
    });
}
...
void UIManager::onRequestAuthorization(const std::string& url, const std::string& code) {
    m_executor.submit([this, url, code]() {
        m_authCheckCounter = 0;
        ThunderLogger::prettyTrace("NOT YET AUTHORIZED");
        std::ostringstream oss;
        ...
        if (m_authState != newState) {
            ...
            switch (m_authState) {
                ...
                case AuthObserverInterface::State::REFRESHED:
                    ThunderLogger::prettyTrace("Authorized!");
                    break;
                case AuthObserverInterface::State::EXPIRED:
                    ThunderLogger::prettyTrace("AUTHORIZATION EXPIRED. RETRYING...");
                    break;
...
void UIManager::printWelcomeScreen() {
    m_executor.submit([]() { ThunderLogger::trace(ALEXA_WELCOME_MESSAGE); });
}

void UIManager::printHelpScreen() {
    m_executor.submit([]() { ThunderLogger::trace(HELP_MESSAGE); });
}

void UIManager::printLimitedHelp() {
    m_executor.submit(
        [this]() { ThunderLogger::trace(LIMITED_HELP_HEADER + m_failureStatus + LIMITED_HELP_MESSAGE); });
}

void UIManager::printSettingsScreen() {
    m_executor.submit([]() { ThunderLogger::trace(SETTINGS_MESSAGE); });
}
...
void UIManager::printLocaleScreen() {
    ...
    auto printLocaleMessage = [supportedLocales, supportedLocaleCombinations]() {
        ...
        ThunderLogger::trace(LOCALE_MESSAGE_HEADER + optionString + LOCALE_MESSAGE_FOOTER);
    };
...
void UIManager::printSpeakerControlScreen() {
    m_executor.submit([]() { ThunderLogger::trace(SPEAKER_CONTROL_MESSAGE); });
}

void UIManager::printFirmwareVersionControlScreen() {
    m_executor.submit([]() { ThunderLogger::trace(FIRMWARE_CONTROL_MESSAGE); });
}

void UIManager::printVolumeControlScreen() {
    m_executor.submit([]() { ThunderLogger::trace(VOLUME_CONTROL_MESSAGE); });
}

#ifdef ENABLE_PCC
void UIManager::printPhoneControlScreen() {
    m_executor.submit([]() { ThunderLogger::trace(PHONE_CONTROL_MESSAGE); });
}

void UIManager::printCallIdScreen() {
    m_executor.submit([]() { ThunderLogger::trace(ENTER_CALL_ID_MESSAGE); });
}

void UIManager::printCallerIdScreen() {
    m_executor.submit([]() { ThunderLogger::trace(ENTER_CALLER_ID_MESSAGE); });
}
#endif

#ifdef ENABLE_MCC
void UIManager::printMeetingControlScreen() {
    m_executor.submit([]() { ThunderLogger::trace(MEETING_CONTROL_MESSAGE); });
}

void UIManager::printSessionIdScreen() {
    m_executor.submit([]() { ThunderLogger::trace(ENTER_SESSION_ID_MESSAGE); });
}

void UIManager::printCalendarItemsScreen() {
    m_executor.submit([]() { ThunderLogger::trace(ENTER_CALENDAR_ITEMS_FILE_PATH_MESSAGE); });
}
#endif

void UIManager::printCommsControlScreen() {
    m_executor.submit([]() { ThunderLogger::trace(COMMS_MESSAGE); });
}

void UIManager::printErrorScreen() {
    m_executor.submit([]() { ThunderLogger::prettyTrace("Invalid Option"); });
}

void UIManager::microphoneOff() {
    m_executor.submit([]() { ThunderLogger::prettyTrace("Microphone Off!"); });
}

void UIManager::printResetConfirmation() {
    m_executor.submit([]() { ThunderLogger::trace(RESET_CONFIRMATION); });
}

void UIManager::printReauthorizeConfirmation() {
    m_executor.submit([]() { ThunderLogger::trace(REAUTHORIZE_CONFIRMATION); });
}

void UIManager::printResetWarning() {
    m_executor.submit([]() { ThunderLogger::prettyTrace(RESET_WARNING); });
}

void UIManager::printDoNotDisturbScreen() {
    m_executor.submit([]() {
        ThunderLogger::trace(DONOTDISTURB_CONFIRMATION_HEADER);
        ThunderLogger::trace(ENABLE_SETTING_MENU);
    });
}

void UIManager::printWakeWordConfirmationScreen() {
    m_executor.submit([]() {
        ThunderLogger::trace(WAKEWORD_CONFIRMATION_HEADER);
        ThunderLogger::trace(ENABLE_SETTING_MENU);
    });
}
...
void UIManager::printSpeechConfirmationScreen() {
    m_executor.submit([]() {
        ThunderLogger::trace(SPEECH_CONFIRMATION_HEADER);
        ThunderLogger::trace(ENABLE_SETTING_MENU);
    });
}

void UIManager::printTimeZoneScreen() {
    m_executor.submit([]() { ThunderLogger::trace(TIMEZONE_SETTING_MENU); });
}
...
void UIManager::onBooleanSettingNotification(
    ...
    m_executor.submit([msg]() { ThunderLogger::prettyTrace(msg); });
}
...
void UIManager::printState() {
    if (m_connectionStatus == avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::Status::DISCONNECTED) {
        ThunderLogger::prettyTrace("Client not connected!");
    } else if (m_connectionStatus == avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::Status::PENDING) {
        ThunderLogger::prettyTrace("Connecting...");
    } else if (m_connectionStatus == avsCommon::sdkInterfaces::ConnectionStatusObserverInterface::Status::CONNECTED) {
        switch (m_dialogState) {
            case DialogUXState::IDLE:
                ThunderLogger::prettyTrace("Alexa is currently idle!");
                return;
            case DialogUXState::LISTENING:
                ThunderLogger::prettyTrace("Listening...");
                return;
            case DialogUXState::EXPECTING:
                ThunderLogger::prettyTrace("Expecting...");
                return;
            case DialogUXState::THINKING:
                ThunderLogger::prettyTrace("Thinking...");
                return;
            case DialogUXState::SPEAKING:
                ThunderLogger::prettyTrace("Speaking...");
                return;
            /*
...
void UIManager::printCommsNotSupported() {
    m_executor.submit([]() { ThunderLogger::trace("Comms is not supported in this device."); });
}
...
void UIManager::onActiveDeviceConnected(const DeviceAttributes& deviceAttributes) {
    m_executor.submit([deviceAttributes]() {
        ...
        ThunderLogger::prettyTrace({"BLUETOOTH DEVICE CONNECTED", "Name: " + deviceAttributes.name, oss.str()});
    });
}
...
void UIManager::onActiveDeviceDisconnected(const DeviceAttributes& deviceAttributes) {
    m_executor.submit([deviceAttributes]() {
        ...
        ThunderLogger::prettyTrace({"BLUETOOTH DEVICE DISCONNECTED", "Name: " + deviceAttributes.name, oss.str()});
    });
}
