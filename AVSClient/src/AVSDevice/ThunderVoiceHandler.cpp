...
bool ThunderVoiceHandler::InteractionHandler::Initialize(std::shared_ptr<InteractionManager> interactionManager)
{
    bool status = true;
...
std::shared_ptr<InteractionManager> ThunderVoiceHandler::InteractionHandler::Manager() const
{
...
void ThunderVoiceHandler::VoiceHandler::Start(const Exchange::IVoiceProducer::IProfile* profile)
{
    ...
    if(m_parent && m_parent->m_interactionHandler) {
        std::shared_ptr<InteractionManager> manager = m_parent->m_interactionHandler->Manager();
        if(manager) {
            manager->holdToggled();
        }
    }
