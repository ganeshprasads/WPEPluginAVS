...
struct IInteractionHandler {
    ...
    virtual bool Initialize(std::shared_ptr<InteractionManager> interactionManager) = 0;
    virtual bool Deinitialize() = 0;
    virtual std::shared_ptr<InteractionManager> Manager() const = 0;
};

/// This class provides and audio input from Thunder
class ThunderVoiceHandler : public applicationUtilities::resources::audio::MicrophoneInterface {
public:
    /**
     ...
     */
    static std::unique_ptr<ThunderVoiceHandler> create(std::shared_ptr<avsCommon::avs::AudioInputStream> stream, WPEFramework::PluginHost::IShell *service, const string& callsign, std::shared_ptr<IInteractionHandler> interactionHandler);
...
private:
     ...
     * @param interactionHandler The interaction handler that will execute necessary action like holdToTalk.
     */
    ThunderVoiceHandler(std::shared_ptr<avsCommon::avs::AudioInputStream> stream, WPEFramework::PluginHost::IShell *service, const string& callsign, std::shared_ptr<IInteractionHandler> interactionHandler);
...
public:
    ...
    public:
        bool Initialize(std::shared_ptr<InteractionManager> interactionManager) override;
        bool Deinitialize() override;
        std::shared_ptr<InteractionManager> Manager() const override;
